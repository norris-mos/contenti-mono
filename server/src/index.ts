...
// this initialises passport authentication middleware in express
...
// This is a function that adds a CSRF token to the user object
const addCsrfTokenToUser = (req, res, next): void => {
  (req.user as any).csrfToken = req.csrfToken;
  next();
};

// checks if the TEMP_UPLOAD directory is true and creates a temp dir if so ( with unsafe cleanup )
