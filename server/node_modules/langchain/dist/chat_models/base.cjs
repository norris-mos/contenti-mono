...
class BaseChatModel extends index_js_2.BaseLanguageModel {
    ...
    async generate(messages, options, callbacks) {
        ...
        const callbackManager_ = await manager_js_1.CallbackManager.configure(callbacks, this.callbacks, parsedOptions.tags, this.tags, { verbose: this.verbose });
        const extra = {
        ...
        // generate results
        const results = await Promise.allSettled(messages.map((messageList, i) => this._generate(messageList, parsedOptions, runManagers?.[i])));
        // handle results
        const generations = [];
        const llmOutputs = [];
