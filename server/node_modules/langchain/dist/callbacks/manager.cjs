    ...
    async handleLLMStart(llm, prompts, _runId = undefined, _parentRunId = undefined, extraParams = undefined) {
        ...
        await Promise.all(prompts.map(async (prompt) => {
            ...
            await Promise.all(this.handlers.map((handler) => (0, promises_js_1.consumeCallback)(async () => {
                if (!handler.ignoreLLM) {
                    try {
                        await handler.handleLLMStart?.(llm, [prompt], runId, this._parentRunId, extraParams, this.tags);
                    }
                    catch (err) {
            ...
            managers.push(new CallbackManagerForLLMRun(runId, this.handlers, this.inheritableHandlers, this.tags, this.inheritableTags, this._parentRunId));
        }));
        return managers;
    }
    async handleChatModelStart(llm, messages, _runId = undefined, _parentRunId = undefined, extraParams = undefined) {
        const managers = [];
        await Promise.all(messages.map(async (messageGroup) => {
            const runId = (0, uuid_1.v4)();
            await Promise.all(this.handlers.map((handler) => (0, promises_js_1.consumeCallback)(async () => {
                if (!handler.ignoreLLM) {
                    try {
                        if (handler.handleChatModelStart)
                            await handler.handleChatModelStart?.(llm, [messageGroup], runId, this._parentRunId, extraParams, this.tags);
                        else if (handler.handleLLMStart) {
                            const messageString = (0, base_js_2.getBufferString)(messageGroup);
                            await handler.handleLLMStart?.(llm, [messageString], runId, this._parentRunId, extraParams, this.tags);
                        }
                    }
                    catch (err) {
                        console.error(`Error in handler ${handler.constructor.name}, handleLLMStart: ${err}`);
