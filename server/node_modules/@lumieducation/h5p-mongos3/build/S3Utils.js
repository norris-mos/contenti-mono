...
var generalizedSanitizeFilename = h5p_server_1.utils.generalizedSanitizeFilename;
var log = new h5p_server_1.Logger('S3Utils');
/**
...
function validateFilename(filename, invalidCharactersRegExp) {
    if (/\.\.\//.test(filename)) {
        log.error("Relative paths in filenames are not allowed: ".concat(filename, " is illegal"));
        throw new h5p_server_1.H5pError('illegal-filename', { filename: filename }, 400);
    }
    if (filename.startsWith('/')) {
        log.error("Absolute paths in filenames are not allowed: ".concat(filename, " is illegal"));
        throw new h5p_server_1.H5pError('illegal-filename', { filename: filename }, 400);
    }
    ...
    // &$@=;:+ ,?\\{^}%`]'">[~<#
    if ((invalidCharactersRegExp !== null && invalidCharactersRegExp !== void 0 ? invalidCharactersRegExp : /[^A-Za-z0-9\-._!()@/]/g).test(filename)) {
        log.error("Found illegal character in filename: ".concat(filename));
        throw new h5p_server_1.H5pError('illegal-filename', { filename: filename }, 400);
    }
}
...
function sanitizeFilename(filename, maxFileLength, invalidCharactersRegExp) {
    return generalizedSanitizeFilename(filename, invalidCharactersRegExp !== null && invalidCharactersRegExp !== void 0 ? invalidCharactersRegExp : /[^A-Za-z0-9\-._!()@/]/g, maxFileLength);
}
